...
<head>
  ...
  <style>
    ...
    .overlay { position:absolute; top:0; left:0; width:100%; height:100%; background: url('${d.logoUrl}') center/contain no-repeat; opacity:0.1; }
...
<body>
  <div class="form-container">
    ...
    <div class="declaration-text">
      <p>أقر أنا <strong>${d.name}</strong> برقم قومي <strong>${d.declarantId}</strong> بأن البيانات الواردة صحيحة على مسؤوليتي الشخصية.</p>
      <p class="declaration-date-right">بتاريخ :${(new Date().toLocaleDateString()).split('/')[2] + '-' + (new Date().toLocaleDateString()).split('/')[0] + '-' + (new Date().toLocaleDateString()).split('/')[1] } </p>
    </div>
...
`;
    ...
    return (
        <div style={{ padding: '1rem', border: '1px solid #ccc' }} className='border rounded rounded-3'>
            <h3>Member Card</h3>
            <ol>
                <li>options {'>'} printer {'('}Destination{')'} {'>'} Printer</li>
                <li>options {'>'} orintaion {'>'} portrait</li>
                <li>options {'>'} paper size {'>'} A4</li>
                <li>options {'>'} margins {'>'} Default</li>
            </ol>
